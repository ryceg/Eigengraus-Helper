...
export class TopCommand extends SlashCommand {
    ...
    async run(ctx) {
        ...
        for (const activity of topMemberResults) {
            index++;
            top5Members += `${index}. <@${activity.member}> - ${await Activity.getPointsForMemberLast30Days(activity.member)} points in the last 30 days\n`;
        }
        index = 0;

        for (const activity of topChannelResults) {
            index++;
            top5Channels += `${index}.${await DiscordUtility.getChannelFromId(activity.channel)} - ${activity.points} points\n`
        }
