...
export class PendingList {
    ...
    public static async getList(id: number): Promise<PendingList> {
        return getConnection().getRepository(PendingList).createQueryBuilder("pendinglist").where("pendinglist.id = :id", {id: id}).getOne();
    }

    public static async updateList(list: PendingList) {
        return getConnection().getRepository(PendingList).createQueryBuilder("pendinglist").update(PendingList).set(list).where("pendinglist.id = :id", {id: list.id}).execute();
    }
