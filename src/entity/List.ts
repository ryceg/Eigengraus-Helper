...
export class List {
  ...
  public static async getList(id: number): Promise<List> {
    return global.CONNECTION.getRepository(List)
      .createQueryBuilder("list")
      .where("list.id = :id", { id: id })
      .getOne()
  }

  public static async getNonTakenList(
    length: number,
    targetMatters: boolean = false
  ): Promise<List> {
    if (targetMatters) {
      return getConnection()
        ...
        .createQueryBuilder("list")
        .where(
          "list.taken = false AND list.finished = false AND list.target = :length",
          { length: length }
        )
        .getOne()
    } else {
      return getConnection()
        .getRepository(List)
        .createQueryBuilder("list")
        .where("list.taken = false AND list.finished = false")
        .getOne()
    }
  }
  ...
  public generateEmbed(finished: boolean = false): MessageEmbed {
    ...
    if (this.items.length !== 0) {
      embed.addField(
        "Items",
        this.items.map((val, index) => `${index + 1}. ${val}`)
      )
    }
    return embed
  }
  ...
  public generateEmbedPaginated(): MessageEmbed {
    ...
    if (this.items.length !== 0) {
      ...
      for (let i = 0; i < totalLength; i += 1024) {
        ...
        embed.addField(
          "Items " + part,
          content.substring(i, Math.min(content.length, i + 1024))
        )
        part++
      }
    }
  ...
  public isFinished(): boolean {
    return (
      this.items.filter((val) => val.length > 0).length === this.items.length
    )
  }
  ...
  public async finish(channel: Channel) {
    ...
    const finishedChannel = await DiscordUtility.getChannelFromName(
      FINISHED_LISTS_CHANNEL
    )
    const arraysChannel = await DiscordUtility.getChannelFromName(
      FINISHED_ARRAYS_CHANNEL
    )
    ...
    if ((await Settings.getSettings()).autoPosting) {
      ...
      await dChannel.send(
        "The list has been finished. Get a new list by using /new"
      )
    }
  }
}
