...
export class Member {
    ...
    static async createMember(member: Member) {
        await getConnection().getRepository(Member).createQueryBuilder("member").insert().into(Member).values([member]).execute();
    }
    ...
    async updateMember() {
        return getConnection().getRepository(Member).createQueryBuilder("member").update(Member).set(this).where("member.id = :id", {id: this.id}).execute();
    }
