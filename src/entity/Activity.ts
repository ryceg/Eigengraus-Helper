...
export class Activity {
  ...
  static async getTopMembers(): Promise<TopMemberResult[]> {
    const rawData = await getConnection().manager.query(
      'select "member", count(*), (select sum(points) from public.activity where activity.ts > (current_date - interval \'30\' day) ) as "points" from (select * from public.activity where activity.ts > (current_date - interval \'30\' day) ) as "foo" group by "member" order by count desc limit 5;  '
    )
  ...
  static async getTopChannels(): Promise<TopChannelResult[]> {
    const rawData = await getConnection().manager.query(
      "select channel, count(*) from activity group by channel;"
    )
  ...
  static async getPointsForMemberLast30Days(member: string): Promise<number> {
    const rawData = await getConnection().manager.query(
      'select sum(points) from activity where "member" = \'' + member + "'"
    )
  ...
  static async getMemberContributions(member: string): Promise<MemberResult> {
    ...
    const topChannelContributionsRaw = await getConnection().manager.query(
      'select channel, sum(points) from activity where "member" = \'' +
        member +
        "' group by channel order by sum desc limit 10;"
    )
    const totPointsRaw = await getConnection().manager.query(
      'select sum(points) from activity where "member" = \'' + member + "'"
    )
    const totPoints7Days = await getConnection().manager.query(
      'select sum(points) from activity where "member" = \'' +
        member +
        "' and ts > (current_date - interval '7' day) ;"
    )
    console.log(
      topChannelContributionsRaw.length,
      totPointsRaw.length,
      totPoints7Days.length
    )
    if (
      topChannelContributionsRaw.length === 0 ||
      totPointsRaw.length === 0 ||
      totPoints7Days.length === 0
    ) {
    ...
    for (let contr in topChannelContributionsRaw) {
      // @ts-ignore
      map.set(
        topChannelContributionsRaw[contr].channel,
        topChannelContributionsRaw[contr].sum
      )
    }
