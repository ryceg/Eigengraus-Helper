...
const { POINTS_PER_ENTRY, POINTS_PER_MESSAGE } = require("../config.json")
...
client.on("message", async (message) => {
  ...
  if (!LIST_REGEX.test(message.content)) {
    ...
    const points =
      anyChannel === null ? POINTS_PER_ENTRY : anyChannel.pointsRate
  ...
  const result = await getConnection()
    ...
    .where("channel.id = :id", { id: message.channel.id })
    .getOne()
  ...
  list.items[index - 2] = CONTENT_REGEX.exec(message.content)[0]
    .substr(index.toString().length)
    .trim()

  const activity = new Activity(
    message.author.id,
    list.bounty + POINTS_PER_ENTRY
  )
  activity.channel = message.channel.id
  await activity.createActivity()
